...
function configureConverter(id) {
    ...
    switch (converter['converter']) {
        ...
        case 'wpc':
            ...
            // - not set (set to be empty). In that case, api-key is null. A "Set" link is displayed
            // - set (but not changed). In that case, new-api-key is empty string (because it is secret). A "Change" link is displayed
            // - changed. In that case new-api-key holds the new api key. A password field is displayed

            //var apiKey = getConverterOption(converter, 'api-key', null);
            var newApiKey = getConverterOption(converter, 'new-api-key', null);
            if (newApiKey == null) {
                document.getElementById('wpc_new_api_key').style.display = 'none';
                document.getElementById('wpc_change_api_key').style.display = 'none';
                document.getElementById('wpc_set_api_key').style.display = 'inline';
            } else {
                document.getElementById('wpc_set_api_key').style.display = 'none';
                if (apiKey == '') {
            ...
            apiVersion = getConverterOption(converter, 'api-version', 2);
...
function updateConverterOptions() {
    ...
    switch (converter['converter']) {
        ...
        case 'wpc':
            ...
            setConverterOption(converter, 'api-version', document.getElementById('wpc_api_version').value);
            setConverterOption(converter, 'api-key', document.getElementById('wpc_new_api_key').value);
