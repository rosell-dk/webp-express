...
function whitelistSetHTML() {
    ...
    if (window.whitelist && window.whitelist.length > 0) {
        ...
        s+='<p><i>No sites have been authorized to use the web service yet.</i></p>';
    }
    s+='<button type="button" class="button button-secondary" id="server_listen_btn" onclick="whitelistAddSite()">+ Authorize website</button>';
...
function whitelistAddWhitelistEntry() {
    ...
    window.whitelist.push({
        ...
        'whitelist-require-api-key-to-be-crypted-in-transfer': document.getElementById('whitelist_require_api_key_to_be_crypted_in_transfer').checked,
//        new_password: '',
        //quota: 60
    });
    updateWhitelistInputValue();
...
function whitelistUpdateWhitelistEntry() {
    ...
    window.whitelist[i]['whitelist-require-api-key-to-be-crypted-in-transfer'] = document.getElementById('whitelist_require_api_key_to_be_crypted_in_transfer').checked;
    whitelistSetHTML();
    closeDasPopup();
}
...
function whitelistEditEntry(i) {
    ...
    document.getElementById('whitelist_api_key').value = '';
    document.getElementById('whitelist_require_api_key_to_be_crypted_in_transfer').checked = entry['whitelist-require-api-key-to-be-crypted-in-transfer'];
